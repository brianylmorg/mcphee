...
\`node -e 'const fs=require("fs"); const stage=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); process.stdout.write(JSON.stringify({action:"stage", householdId:process.env.HOUSEHOLD_ID, babyId:process.env.BABY_ID, sourceNote:"Mistral paper-log gated import", createdBy:"JY", rows:stage.rows}))' ${out} | curl -sS -X POST "$MCPHEE_BASE_URL/api/admin/import-paper-log?key=$MIGRATION_KEY" -H 'content-type: application/json' --data-binary @-\`

Commit command template, after reviewing staged rows in McPhee and marking rows reviewed:\n\n\`curl -sS -X POST "$MCPHEE_BASE_URL/api/admin/import-paper-log?key=$MIGRATION_KEY" -H 'content-type: application/json' --data '{"action":"commit","batchId":"'$BATCH_ID'","createdBy":"JY","confirmation":"I reviewed these rows against the source paper logs"}'\`\n`;
